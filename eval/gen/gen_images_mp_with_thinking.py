...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_images", type=int, default=1) #4)
    ...
    for idx in range(start, end):
        ...
        for i in range(args.num_images // args.batch_size):
            if args.think:
                tmp_image, think_output = generate_image_with_think(
                    prompt=prompt,
                    cfg_scale=cfg_scale, 
                    cfg_interval=cfg_interval, 
                    cfg_renorm_min=cfg_renorm_min,
                ...
                tmp_image_list = [tmp_image]
                tmp_thought_list = [think_output]
            else:
            ...
            if args.think:
                thought_list.extend(tmp_thought_list)

        sample_count = 0
        if args.think:
            for sample, thought in zip(image_list, thought_list):
                sample = sample.crop(sample.getbbox())
                new_sample_path = os.path.join(sample_path, f"{sample_count:05}.png")
                sample.save(new_sample_path)
                sample_count += 1
                with open(new_sample_path.replace(".png", ".txt"), "w") as f:
